...
Use the consent event in your dataLayer to fire tags only when consent is explicitly granted. This approach requires the dataLayer consent pattern from [Consent State via DataLayer](/consent/consent-state-datalayer/).
...
If you are pushing consent state to the dataLayer (as described in [Consent State via DataLayer](/consent/consent-state-datalayer/)), you can use a simpler Data Layer Variable instead of the Custom JavaScript approach:
...
<CardGrid>
  <LinkCard title="Consent State via DataLayer" href="/consent/consent-state-datalayer/" description="The dataLayer consent pattern that enables the dual-trigger approach." />
  <LinkCard title="Testing Consent" href="/consent/testing-consent/" description="How to verify non-Google tags are correctly blocked without advertising consent." />
  <LinkCard title="Consent Mode GTM Setup" href="/consent/consent-mode-gtm-setup/" description="Setting up consent requirements on Google tags as a complement to non-Google gating." />
</CardGrid>
