...
<Aside type="tip">
New to SQL or to the GA4 export schema specifically? Read the [SQL Primer for GA4 BigQuery](/ga4/bigquery/sql-primer/) first — it covers basic `SELECT`/`WHERE`/`GROUP BY`, the minimal `UNNEST` pattern, CTEs, and partition pruning. Everything on this page builds on that foundation.
</Aside>
...
### Extracting user property values
...
### UNNEST with full property details
