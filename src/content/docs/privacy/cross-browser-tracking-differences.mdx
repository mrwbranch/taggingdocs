...
Global browser market share (desktop + mobile combined, current as of 2026):
- Chrome: ~65%
- Safari: ~19%
- Edge: ~5%
- Firefox: ~3%
...
**Link Tracking Protection (iOS 17+, expanded through iOS 26):** Safari strips known tracking click IDs (`fbclid`, `mc_eid`, `ttclid`, `hsCtaTracking`, and — as of the iOS 26 era — `gclid`, `gbraid`, `wbraid` in most contexts) from URLs in Private Browsing, Mail, Messages, and default Safari under Advanced Tracking and Fingerprinting Protection. `utm_*` parameters are preserved. See [ITP History & Impact](/privacy/itp-history-impact/) for the rollout timeline and mitigations (server-side enhanced conversions, first-party data capture).
...
**Total Cookie Protection (default for all users since Firefox 103, 2022):** Firefox's Total Cookie Protection partitions cookies by site. A tracker's cookie set on Site A is separate from the same tracker's cookie on Site B. This prevents cookie-based cross-site tracking without blocking cookies entirely. It is more sophisticated than a simple block and now applies in Standard mode for every Firefox user.

**Privacy-Preserving Attribution (PPA, Firefox 128+, 2024):** Firefox ships an experimental attribution API (on by default, user-disableable) that lets advertisers measure conversions from ad impressions/clicks with differential-privacy-level aggregation, rather than cross-site cookies. Similar in spirit to Chrome's Attribution Reporting API and Apple's Private Click Measurement. Not widely adopted by ad platforms yet, but the plumbing is live in the browser.
...
## Chrome: Privacy Sandbox (3PC deprecation abandoned)

Chrome spent years planning to deprecate third-party cookies entirely. That plan was **abandoned in July 2024**. Google announced it would not force third-party cookie removal; instead, it would offer a new user-choice surface and continue investing in Privacy Sandbox APIs as opt-in alternatives.

**Current state (2026):**

- **Third-party cookies are still enabled by default in Chrome.** There is no mandated deprecation deadline.
- Users can opt out of third-party cookies via a prompt/setting, but the default remains "allow."
- Privacy Sandbox APIs (Topics, Protected Audience, Attribution Reporting) continue to ship as optional replacements for advertisers who want to move off third-party cookies voluntarily.
- Enterprise policy and parental controls can still disable third-party cookies at the deployment level.

**Privacy Sandbox APIs (opt-in alternatives):**
...
**Impact on GA4 in Chrome:**

- GA4 does not rely on third-party cookies for core analytics — it uses first-party cookies — so Chrome is the least restrictive major browser for GA4 measurement accuracy.
- Google Ads remarketing and cross-site attribution that did depend on third-party cookies now have a longer runway than expected. You can keep using third-party cookies in Chrome for the foreseeable future, but **adopting server-side conversion APIs and first-party identity strategies is still the right move** — it makes you resilient to Safari/Firefox restrictions that are not going away, and to any future Chrome policy shift.

**Chrome's first-party cookie behavior:**
Chrome does not impose caps on JavaScript-set first-party cookies like Safari does. `_ga` retains its full 2-year lifetime in Chrome.
