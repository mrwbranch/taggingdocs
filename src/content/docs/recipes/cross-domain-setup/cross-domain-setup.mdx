...
<Aside type="tip">
Cross-domain tracking is NOT needed for subdomains of the same root domain (e.g., `shop.example.com` and `blog.example.com`). GA4 handles subdomains automatically with the default cookie configuration. See [Subdomain Tracking](/recipes/cross-domain-setup/subdomain-tracking/) for that scenario.
</Aside>
...
<CardGrid>
  <LinkCard title="Subdomain Tracking" href="/recipes/cross-domain-setup/subdomain-tracking/" description="Track across subdomains — a much simpler scenario." />
  <LinkCard title="iFrame Tracking" href="/recipes/cross-domain-setup/iframe-tracking/" description="When the other domain is embedded as an iframe." />
</CardGrid>
