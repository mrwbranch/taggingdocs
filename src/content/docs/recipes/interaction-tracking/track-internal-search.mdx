...
**Search query contains PII.** If users search for their own email addresses or account numbers, those values flow into GA4. Consider scrubbing PII patterns from `search_term` before pushing. See the [PII Detection recipe](/recipes/privacy-consent/pii-detection-removal/).
...
<CardGrid>
  <LinkCard title="Track 404 Errors" href="/recipes/interaction-tracking/track-404-errors/" description="Another server-side signal that benefits from the dataLayer approach." />
  <LinkCard title="PII Detection & Removal" href="/recipes/privacy-consent/pii-detection-removal/" description="Strip sensitive data from tracking events before they leave the browser." />
</CardGrid>
