...
<Aside type="note">
GTM-based A/B tests have a flash of original content (FOOC) problem: the page loads first in its default state, then GTM applies the variant after loading. Use the anti-flicker snippet to prevent this. For tests involving significant layout changes, a purpose-built testing tool is more appropriate, or consider [server-side variant assignment](/recipes/testing/server-side-experiments/) which eliminates flicker entirely.
</Aside>

<Aside type="tip">
Before running any test, work through [Experimentation Fundamentals](/recipes/testing/experimentation-fundamentals/) (hypothesis, primary metric, guardrails) and [Sample Size and MDE](/recipes/testing/sample-size-and-mde/) (how much traffic you actually need). The tactical mechanics below don't produce useful results without the methodology behind them. See also [Flicker and FOUC Prevention](/recipes/testing/flicker-and-fouc-prevention/) for the anti-flicker snippet in depth.
</Aside>
...
<CardGrid>
  <LinkCard title="A/B Test Tracking" href="/recipes/testing/ab-test-tracking/" description="Integrating data from professional A/B testing tools with GA4." />
  <LinkCard title="User-ID Implementation" href="/recipes/testing/user-id-implementation/" description="Consistent user identification for cross-device test attribution." />
</CardGrid>
